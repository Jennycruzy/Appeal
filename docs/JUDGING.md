...
| Durable asynchronous workflow | Verified locally; hosted continuation open | Restart-safe evidence and payer wakes, duplicate IDs, deadline sentinel, and receipt continuity |
...
| Case survives restart and external wakes it | `make run-async-workflow-proof` | [`evidence/async-workflow-proof.json`](../evidence/async-workflow-proof.json), [`docs/audits/durable-async-workflow.md`](audits/durable-async-workflow.md). This is a local deterministic restart harness; a fresh hosted evidence/payer wake and end-to-end trace are still required. |
...
| Payer determination cannot mutate directly | Local typed payer event and `LocalCaseRuntime.handle_event` | [`src/appeal_platform/payer.py`](../src/appeal_platform/payer.py), [`src/appeal_platform/runtime.py`](../src/appeal_platform/runtime.py), tests in [`tests/test_platform.py`](../tests/test_platform.py). A separate payer Cloud Run service/account is not deployed yet. |
